...
notes, *deltas-other-concentration*, Secs. 4–5):
...
| 2 | + a location–scale shape | `location_scale.py` | `gaussian` (exact; `band='mc'` tighter), `location_scale` (logistic, *t*, laplace) | high-probability | keeps falling |
...
below about ln(1/δ)/N (notes, Props. 2–3). To give a scarce class room that
...
`closed_form_minimax` (notes, Prop. 4): with the same tail on both sides, the
minimax boundary does not depend on the tail shape (Prop. 5).
