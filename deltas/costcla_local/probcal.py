...
class ROCConvexHull:
    ...
    def fit(self, y, p):
        ...
        def prob_freq(y, predict_proba):
            ...
            freq_0_tempb = np.bincount(freq_0_tempa.searchsorted(
                predict_proba[np.nonzero(y == 0)[0]]))
            freq = np.zeros((proba_bins.shape[0], 3))
            freq[:, 0] = proba_bins
            for i in range(freq_0_tempa.shape[0]):
                freq[np.nonzero(proba_bins == freq_0_tempa[i])
                     [0], 1] = freq_0_tempb[i]
        ...
        for i in range(rocch_thresholds.shape[0] - 1):
            ...
            join_elements = np.logical_and(
                f_cal[i, 1] <= f[:, 0], f_cal[i, 0] > f[:, 0])
