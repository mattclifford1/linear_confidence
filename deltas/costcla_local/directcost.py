...
from deltas.costcla_local.probcal import ROCConvexHull
from deltas.costcla_local.metrics import cost_loss
...
class BayesMinimumRiskClassifier(BaseEstimator):
    ...
    def predict(self, y_prob, cost_mat=None):
        ...
        t_bmr = (cost_mat[:, 0] - cost_mat[:, 3]) / (cost_mat[:,
                                                              1] - cost_mat[:, 3] - cost_mat[:, 2] + cost_mat[:, 0])
